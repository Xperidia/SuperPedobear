...
killicon.Add("spb_seeker", "superpedobear/bear", Color(255, 255, 255, 255))
...
local function DrawDeath(x, y, death, hud_deathnotice_time)
	...
	-- Draw KILLER
	if death.left then
		draw.SimpleText(death.left, "spb_HUDname", x - ( w / 2 ) - 17, y + 1, Color(0, 0, 0, alpha), TEXT_ALIGN_RIGHT)
		draw.SimpleText(death.left, "spb_HUDname", x - ( w / 2 ) - 16, y, death.color1, TEXT_ALIGN_RIGHT)
	end

	-- Draw VICTIM
	draw.SimpleText(death.right, "spb_HUDname", x + (w / 2) + 17, y + 1, Color(0, 0, 0, alpha), TEXT_ALIGN_LEFT)
	draw.SimpleText(death.right, "spb_HUDname", x + (w / 2) + 16, y, death.color2, TEXT_ALIGN_LEFT)
